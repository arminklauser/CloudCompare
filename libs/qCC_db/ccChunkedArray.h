...
template <class Type, int N, class ComponentType> class ccChunkedArray : public std::vector<Type>, public CCShareable, public ccHObject
{
public:

	//! Base type
	typedef ccChunkedArray<Type, N, ComponentType> Base;
	...
	bool copy(Base& dest) const
	{
		...
		{
			ccLog::Warning("[ccChunkedArray::copy] Not enough memory");
			return false;
		}
		return true;
	}
	...
	bool resizeSafe(size_t count, bool initNewElements = false, const Type* valueForNewElements = nullptr)
	{
		try
		{
			if (initNewElements)
			{
				...
				{
					ccLog::Warning("[ccChunkedArray::resizeSafe] Internal error: no new element specified");
					return false;
				}
...
protected:
	...
	virtual ~ccChunkedArray() {}

	//inherited from ccHObject
	virtual bool toFile_MeOnly(QFile& out) const override { return ccSerializationHelper::GenericArrayToFile<Type, N, ComponentType>(*this, out); }
	virtual bool fromFile_MeOnly(QFile& in, short dataVersion, int flags) override { return ccSerializationHelper::GenericArrayFromFile<Type, N, ComponentType>(*this, in, dataVersion); }
